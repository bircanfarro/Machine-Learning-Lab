...
df_orders <- read.csv("./data/Orders.csv", stringsAsFactors = F)
...
betterDates <- as.Date(df_orders$Order.Date,format = "%m/%d/%y")
#df_orders %>% mutate(Order.Date1=)

df_orders$Order.Date <- format(as.Date(df_orders$Order.Date), "%Y/%m/%d")
...
```{r}
df_both=merge(df_orders,df_returns,by='Order.ID', all.x = TRUE)

# converting NA in return columns to 'No'
df_both$Returned[is.na(df_both$Returned)]='No'
...
df_both$Ship.Date <- as.Date(df_both$Ship.Date , "%m/%d/%y")
df_both$Order.Date <- as.Date(df_both$Order.Date , "%m/%d/%y")
df_both$Process.Time <- df_both$Ship.Date-df_both$Order.Date
View(df_both)
```
...
```{r}
#unique(df_both$Product.ID)
df_both %>% group_by(Product.ID) %>% count(returned_yes=Returned=='Yes') 


```

```{r}
#summarise(Returned=TRUE) 

df_both1=df_both %>%
  group_by(Product.ID) %>%
  mutate(unique = n_distinct(Returned))
...
set.seed(0)
train_ind = sample(1:nrow(df_both1), 8*nrow(df_both1)/10) #Training indices.
train = df_both1[train_ind, ] # Train dataset
test = df_both1[-train_ind, ] #Test dataset.( only features)
#High.test = High[-train] #Test response columns
```

```{r}

#Ftting and visualizing a classification tree to the training data.
tree.carseats = tree(Returned~ . - Order.ID, data = df_both1, subset = train)
plot(tree.carseats)
